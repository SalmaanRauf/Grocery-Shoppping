...
std::size_t GroceryList::size() const
{
  ...
  ///////////////////////// TO-DO (1) //////////////////////////////
    ...
    // easier to grab the size of array/vector rather than SLL or DLL
...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )        // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ///////////////////////// TO-DO (4) //////////////////////////////
      ...
      if( _gList_array_size /* valid elements */ >= _gList_array.size() /* capacity */ ) throw CapacityExceeded_Ex( "Cannot fit another grocery item into fixed sized array already at capacity" exception_location );
      
      std::move_backward( _gList_array.begin() + offsetFromTop,
                          _gList_array.begin() + _gList_array_size, //shifts every element but last over 1
                          _gList_array.begin() + _gList_array_size + 1 );


      _gList_array[offsetFromTop] = groceryItem; //fill in the gap with new grocery item
      //must increment size, since we inserted somthing 
      ++ _gList_array_size;
    /////////////////////// END-TO-DO (4) ////////////////////////////
  } // Part 1 - Insert into array
  ...
  { /**********  Part 2 - Insert into vector  **********************/
    ...
    //                  start at beginnning, add offset from the top, then insert the grocery item
   _gList_vector.insert( _gList_vector.begin() + offsetFromTop, groceryItem );
    // vector will self manage size and location and whatnot 
  ...
  { /**********  Part 3 - Insert into doubly linked list  **********/
    ///////////////////////// TO-DO (6) //////////////////////////////
      ...
      //must walk the list, move offsetfrom top from the beginning, offsetfromtop is the amount you wan tto walk.
  ...
  { /**********  Part 4 - Insert into singly linked list  **********/
    ///////////////////////// TO-DO (7) //////////////////////////////
      ...
      //cannot insert before, SLL only move forward
      //must find node before the one we want to add after
      //must use insert after for that reason
...
void GroceryList::remove( std::size_t offsetFromTop )
{
  ...
  { /**********  Part 2 - Remove from vector  **********************/
    ///////////////////////// TO-DO (9) //////////////////////////////
      ...
      /// array above.
    //validating the position parameter
    if (offsetFromTop < _gList_vector.size())
    {
      auto vector_insertionIterator = _gList_vector.begin() + offsetFromTop;
    
    // Use the erase function to remove the element from the vector
    _gList_vector.erase(vector_insertionIterator);
    }
  ...
  // Convert offsetFromTop to position to pointer to remove
  auto positionToRemove = _gList_dll.begin();
  std::advance(positionToRemove, offsetFromTop);

  // erase function to remove desired
  _gList_dll.erase(positionToRemove);
    /////////////////////// END-TO-DO (10) ////////////////////////////
  } // Part 3 - Remove from doubly linked list
  ...
  {/**********  Part 4 - Remove from singly linked list  **********/
    ///////////////////////// TO-DO (11) //////////////////////////////
      ...
      /// advancing _gList_sll.before_begin() offsetFromTop times.  The STL has a function called std::next() that does that, or you
      /// can write your own loop.
      //converting offsetfromtop to iterator pointing to the position before the element we remove
    auto positionBeforeRemove = _gList_sll.before_begin();
    for (std::size_t i = 0; i < offsetFromTop; ++i) {
      std::advance(positionBeforeRemove, 1);
  }

  // erase_after because SLL can only move forward
  _gList_sll.erase_after(positionBeforeRemove);
...
void GroceryList::moveToTop( const GroceryItem & groceryItem )
{
  ...
  std::size_t position = find(groceryItem);

  //if it exists then procceed to remove and reinsert
...
GroceryList & GroceryList::operator+=( const std::initializer_list<GroceryItem> & rhs )
{
  ...
  for (const auto &item : rhs) {
    insert(item, Position::BOTTOM);//insert each item at the bottom
  }
  /////////////////////// END-TO-DO (13) ////////////////////////////
...
std::weak_ordering GroceryList::operator<=>( GroceryList const & rhs ) const
{
  ...
  //finding common extent
  std::size_t commonExtent = std::min(size(), rhs.size());

  //iterate through the common extent and compare the items
  for (std::size_t i =0; i < commonExtent; ++i)
  {
    std::weak_ordering result = _gList_array[i] <=> rhs._gList_array[i];
       if (result != 0) return result;
  }
  //if theres no diff in common extent, compare the sizes
...
bool GroceryList::operator==( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (16) //////////////////////////////
  //checking if sizes are different

  if (_gList_array_size != rhs._gList_array_size){
    return false;
  }

  //find items that don't match together, going to use != in case DLL or SLL
  for(std::size_t i = 0; i< _gList_vector.size(); ++i)
  {
...
std::size_t GroceryList::gList_sll_size() const
{
  ...
  std::size_t count = 0;
  //use a loop to find the distance between begin and end
  for(auto itSize = _gList_sll.cbegin(); itSize != _gList_sll.cend(); ++itSize) 
  {
    ++count;
  }
...
//read the grocery items one by one and add to grocery list
  GroceryItem newItem;

  while(stream >> newItem){
  groceryList.insert(newItem, GroceryList::Position::BOTTOM);
  }
  /////////////////////// END-TO-DO (18) ////////////////////////////
